 ...
 *******************************************************/
static const char *MESH_TAG = "mesh_node";
static const uint8_t MESH_ID[6] = { 0x77, 0x77, 0x77, 0x77, 0x77, 0x90};
...
void esp_mesh_p2p_tx_main(void *arg)
{
    ...
    while (is_running) {
        ...
        if (!esp_mesh_is_root()) {
            ...
            if (statsFlag)
            {
                sprintf(sensorMsg,"%.2f,%.3f,%.3f,%f",temperature,mean(),median(),variance(mean()));
                memcpy(tx_buf, sensorMsg, sizeof(sensorMsg));
                err = esp_mesh_send(NULL, &data, NULL, NULL, 0);
            }
...
void app_main(void)
{
    ...
    memcpy((uint8_t *) &cfg.mesh_ap.password, AP_PASSWD,
           strlen(AP_PASSWD));
